#!/bin/bash
# Multi-Node + Multi-GPU parallel inference using qwen3_vl_infer_all.py
...
# Total workers = all GPUs across all nodes
TOTAL_WORKERS=$((NNODES * NUM_GPUS))
# This node is responsible for shard indices [MY_SHARD_START, MY_SHARD_END)
