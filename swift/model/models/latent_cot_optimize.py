...
def compute_explain_loss(
    ...
    for b in range(batch_size):
        ...
        for step_idx in range(min(n_latent_groups, len(step_ids))):
            ...
            combined_embeds = torch.cat(parts, dim=0)
            embeds_batch.append(combined_embeds)
            logger.info_once(
            ...
            labels_batch.append(torch.full(
                (combined_embeds.shape[0], ), -100,
                dtype=torch.long, device=last_hidden_states.device))
            labels_batch[-1][prefix_len:] = step_tensor

            attn_mask = torch.ones(
                (combined_embeds.shape[0], ),
                dtype=torch.long, device=last_hidden_states.device)
            mask_batch.append(attn_mask)

    combined_embeds = torch.nn.utils.rnn.pad_sequence(embeds_batch, batch_first=True)
    attn_mask = torch.nn.utils.rnn.pad_sequence(mask_batch, batch_first=True)
    ...
    logits = aux_outputs.logits if hasattr(aux_outputs, 'logits') else aux_outputs
    for b in range(logits.shape[0]):
        shift_logits = logits[b][..., :-1, :].contiguous()
        shift_labels = labels_batch[b][..., 1:].contiguous()
...
def compute_visual_explain_loss(
    ...
    for b in range(batch_size):
        ...
        labels_batch.append(torch.full(
            (combined_embeds.shape[0], ), -100,
            dtype=torch.long, device=last_hidden_states.device))
        labels_batch[-1][prefix_len:] = target_tensor
    ...
    logits = vis_aux_outputs.logits if hasattr(vis_aux_outputs, 'logits') else vis_aux_outputs
    for b in range(logits.shape[0]):
        shift_logits = logits[b][..., :-1, :].contiguous()
        shift_labels = labels_batch[b][..., 1:].contiguous()
        shift_logits = shift_logits[:shift_labels.shape[0], ...]
